const { createTableClient, getUserInfo, isAdmin, ensureUser } = require("../shared/tableClient");

module.exports = async function (context, req) {
    context.log('UpdateUser function processing request.');

    const clientPrincipal = getUserInfo(req);
    if (!clientPrincipal || !clientPrincipal.userId) {
        context.res = { status: 401, body: "User not authenticated." };
        return;
    }

    await ensureUser(req);

    const adminStatus = await isAdmin(clientPrincipal.userId);
    if (!adminStatus) {
        context.res = { status: 403, body: "Admin access required." };
        return;
    }

    const { action, userId } = req.body || {};

    if (!action || !userId) {
        context.res = { status: 400, body: "Missing required fields: 'action' and 'userId'." };
        return;
    }

    try {
        const usersClient = createTableClient("users");

        if (action === 'toggleAdmin') {
            // Prevent removing yourself as admin
            if (userId === clientPrincipal.userId) {
                context.res = { status: 400, body: "Cannot toggle your own admin status." };
                return;
            }

            // Get current user record
            let entity;
            try {
                entity = await usersClient.getEntity("users", userId);
            } catch (e) {
                if (e.statusCode === 404) {
                    context.res = { status: 404, body: `User '${userId}' not found.` };
                    return;
                }
                throw e;
            }

            const newAdminStatus = !(entity.isAdmin === true);
            await usersClient.upsertEntity({
                partitionKey: "users",
                rowKey: userId,
                isAdmin: newAdminStatus
            }, "Merge");

            context.res = {
                status: 200,
                body: { message: `User '${userId}' admin status set to ${newAdminStatus}.`, isAdmin: newAdminStatus }
            };
        } else {
            context.res = { status: 400, body: "Invalid action. Use 'toggleAdmin'." };
        }
    } catch (err) {
        context.log.error("Error updating user:", err);
        context.res = { status: 500, body: `Error updating user: ${err.message}` };
    }
};
